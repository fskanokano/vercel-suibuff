...
// Vercel 适配层
// ---------------------------------------------------------------------------
// 原文件是 Cloudflare Worker 格式：export default { fetch(request, env) }。
// Vercel 的 Node.js 运行时（@vercel/node >= 5）原生识别该形态 —— 只要导出的
// 对象带 fetch 方法，就会自动把 Node IncomingMessage 桥接成标准 Web Request
// 调用 fetch(request)，并把返回的 Web Response 流式写回（SSE 无损）。
//
// 唯一差异：CF Worker 由平台注入 env 参数，而 Vercel 调用 fetch 只传 request。
// 因此这里用 process.env 构造等价 env（变量名与 CF Worker 完全一致），
// 在 Vercel 项目 Settings → Environment Variables 里配置同名变量即可：
//
//   FREEBUFF_TOKEN    必需：freebuff token，多账号用换行/逗号分隔
//   API_KEY           可选：访问 API key（缺省 freebuff-default-key）
//   FREEBUFF_API_KEY  可选：同 API_KEY 的别名，两者取其一
...
export default {
  async fetch(request) {
    return workerFetch(request, envShim);
  },
};
